...
```SQL
	...
	ВЫБРАТЬ
		...
		СУММА(ТаблицаПродаж.Сумма) КАК СуммаНарастающимИтогом 
	...
	ПО    
		Календарь.Период >= ТаблицаПродаж.Период
	СГРУППИРОВАТЬ ПО
		Календарь.Период 
	;
	...
	ВЫБРАТЬ 
		...
		0 КАК Период,        
		0 КАК СуммаНарастающимИтогомСНачалаГода
	ИЗ
	...
	ВЫБРАТЬ
		0 КАК Контрагент,
		СуммаПродаж.Период КАК Период,
		СуммаПродаж.СуммаНарастающимИтогом КАК СуммаНарастающимИтогомСНачалаГода
	ИЗ
...
![5new](https://user-images.githubusercontent.com/36735568/192326073-c8cfb142-9ad1-43dd-a37b-2ba0523682f5.png)
